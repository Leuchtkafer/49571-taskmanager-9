...
export class Statistic extends AbstractComponent {
  ...
  _createDaysChart() {
    ...
    const sortDates = this._tasks.map((element) => moment(element.dueDate).format(`DD MMM`)).sort();
  ...
  _createTagsChart() {
    ...
    const allTags = this._tasksInTag(this._tasks);
  ...
  _createColorsChart() {
    ...
    const sortColors = this._tasks.map((element) => element.color).sort();
